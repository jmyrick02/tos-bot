...
def save(guild_id):
    game = games[guild_id]
    data = [str(guild_id), str(game.in_progress), str(game.game_channel_id), str(game.player_role_id), str(game.transition_times[0][0]), str(game.transition_times[0][1]), str(game.transition_times[1][0]), str(game.transition_times[1][1]), str(game.day)]
...
def load(guild_id):
    ...
    try:
        ...
        transition_times = [[int(data[4]), int(data[5])], [int(data[6]), int(data[7])]]
        day = float(data[8])
